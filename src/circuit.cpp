...
layeredCircuit layeredCircuit::randomize(int layerNum, int eachLayer)
{
    ...
    {
        c.circuit[0].gates[i] = gate(gateType::Input, 0, F::random(), 0, F_ZERO, false);
    }
    for (int i = 1; i < layerNum; ++i)
    {
        ...
        {
            c.circuit[i].gates[j] = gate((F::random() & 1) == 0 ? gateType::Add : gateType::Mul, F::random() % i, F::random() % gateSize, F::random() % gateSize, F_ZERO, false);
        }
    }
    return c;
}
